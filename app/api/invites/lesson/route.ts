...
export async function POST(request: NextRequest) {
  try {
    ...
    const { lesson_id, course_id, expires_at, max_uses } = body;
    ...
    return NextResponse.json({
      invite: {
        ...
        lesson: invite.lessons,
        course: invite.courses,
        invite_url: `${process.env.NEXT_PUBLIC_APP_URL}/join/lesson/${invite.invite_token}`,
      },
    });
  } catch (error) {
    console.error("Error creating lesson invite:", error);
    return NextResponse.json(
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Error fetching lesson invites:", error);
    return NextResponse.json(
      { error: "Failed to fetch lesson invites" },
      { status: 500 }
    );
  }
}
